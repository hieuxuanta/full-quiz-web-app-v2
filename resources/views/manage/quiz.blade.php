@extends('layouts.app')
@section('title', 'Manage Quiz - BQuiz')
@section('content')
    <style>
        body {
            padding-top: 90px;
        ...
        <div class="row">
            <div class="col-lg-9">
                ...
                <p>Class:
                    <b>
                        <a
                            href="/manage/class/view{{ $quiz_details->class_id }}"></a>{{ $quiz_details->classe->course_sec }}</b>
                </p>
                <p>Subject:
                ...
                <p>Questionnaire:
                    <b>
                        <a
                            href="/questionnaire/{{ $quiz_details->questionnaire->questionnaire_id }}">{{ $quiz_details->questionnaire->questionnaire_name }}</a>
                    </b>
                </p>
            </div>
            ...
            <div class="col-lg-3" id="quiz_actions">
                ...
                @if ($quiz_details->quiz_event_status == 0)
                    <button href="" onclick="javascript:ChangeQuizStatus({{ $quiz_details->quiz_event_id }}, 1)"
                        class="btn btn-primary">Enable Quiz</button>
                @elseif($quiz_details->quiz_event_status == 1)
                    <button href="" onclick="javascript:ChangeQuizStatus({{ $quiz_details->quiz_event_id }}, 0)"
                        class="btn btn-primary">Disable Quiz</button>
                    <button href="" onclick="javascript:ChangeQuizStatus({{ $quiz_details->quiz_event_id }}, 2)"
                        ...
                        @foreach ($results->classe->student_class as $result)
                            <tr>
                                ...
                                <td>
                                    ...
                                    {{ $result->user_profile->usr_identification_numb }}
                                </td>
                                <td>
                                    @if (is_null($result->student_score))
                                        <i>not taken</i>
                                    @else {{ $result->student_score->score }} / {{ $sum }}
                                    @endif
                                </td>
                                <td>
                                    @php
                                        try {
                                            $ave = $result->student_score->score / $sum;
                                        } catch (Exception $e) {
                                            $ave = 0;
                                        }
